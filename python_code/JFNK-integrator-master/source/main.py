...
print("Y_all 结构:")
print("时间步总数:", len(Y_all))
for i, Y in enumerate(Y_all):
    print(f"  步 {i}: {len(Y)} 次迭代, 每次迭代解的形状 {[y.shape for y in Y]}")
...
print("\nt:")
# print(t_all.T)
# print(t_all[np.newaxis, :])
print(t_all.reshape(-1, 1))
...
plot_errors([y_all, y_exact], titles=["Numerical Solution", "Exact Solution"], labels=["Numerical Solution", "Exact Solution"], main_title="Numerical Solution vs Exact Solution", xlabel="Index", ylabel="Solution", ls='-o')
plt.show(block=False)
# 使用 plotter.py 中的绘图函数绘制误差分析图
plot_errors([aerr, rerr], titles=["Absolute Error", "Relative Error"], labels=["Absolute Error", "Relative Error"], main_title="Error Analysis", xlabel="Index", ylabel="Error")
plt.show(block=False)
