...
run_sdc_sweep_num = 0
run_jfnk_initial_num = 0
run_jfnk_iteratin_num = 0
    
def get_u_from_x(xnow, tnow):

    return vs(tnow)

def vs(t):

    Vrms = 735e3 / np.sqrt(3)  # 相电压有效值
    f = 60                      # 频率 60Hz
    deg_phase_shift = 0         # 相位偏移（度）
    
    u = np.sqrt(2) * Vrms * np.sin(2 * np.pi * f * t + deg_phase_shift / 180 * np.pi)
    return u
...
def backward_euler(f_eval, t, y0,A_matrix, B_matrix, S, be_tol=BE_TOL, do_print=False):
    ...
    for i in range(n_nodes):
        ...
        if i == 0 and h == 0:
            ...
            y[i, :] = backward_euler_node(f_eval, t=t[i], h=h, rhs=rhs, y0=rhs,A_matrix=A_matrix, B_matrix=B_matrix, be_tol=be_tol)
            # print("i: ", i)
            # print("rhs:", rhs)
            # print("y(i,:): ", backward_euler_node(f_eval, t=t[i], h=h, rhs=rhs, y0=rhs,A_matrix=A_matrix, B_matrix=B_matrix, be_tol=be_tol))
...
# 线性系统求解
# def backward_euler_node(f_eval, t, h, rhs, y0,A_matrix, B_matrix, be_tol=BE_TOL):
...
#     y0 = y0.flatten()
#     m_size = len(rhs)
#     I = eye(m_size, format='csr')
#     ut = get_u_from_x(y0, t)

#     system_matrix = I - h * A_matrix
    
#     # 关键修复：确保所有操作都在相同维度
#     # 将rhs转为列向量，B*u(t)的结果也是列向量
#     rhs_column = rhs.reshape(-1, 1)
    
#     # 计算 B * u(t)
#     if hasattr(B_matrix, 'dot'):
#         Bu = B_matrix.dot(ut).reshape(-1, 1)  # 确保是列向量
#     else:
#         Bu = (B_matrix @ ut).reshape(-1, 1)   # 确保是列向量
    
#     right_hand_side = rhs_column + h * Bu
    
#     y = spsolve(system_matrix, right_hand_side)
...
def backward_euler_node(f_eval, t, h, rhs, y0, A_matrix, B_matrix, be_tol=BE_TOL):
    """
    Python版本的backward_euler_node函数，使用牛顿迭代法求解后向欧拉节点
    
    :param function f_eval: 导数函数 f(t, y)
    :param float t: 当前时间
    :param float h: 步长
    :param numpy.ndarray rhs: 右端项
    :param numpy.ndarray y0: 初始猜测解
    :param numpy.ndarray A_matrix: 系统矩阵A
    :param numpy.ndarray B_matrix: 输入矩阵B  
    :param float be_tol: 收敛容差
    :param bool do_print: 是否打印调试信息
    ...
    while iter_count < N_ITER_MAX_NEWTON:
        ut = get_u_from_x(y, t)
        # print("A_matrix.shape:", A_matrix.shape)
        # print("y.shape:", y.shape)
        # print("B_matrix.shape:", B_matrix.shape) 
        # print("B_matrix.transpose().shape:", B_matrix.transpose().shape)
        # print("ut.reshape(-1, 1).shape:", ut.reshape(-1, 1).shape)
        f_val = A_matrix @ y.reshape(-1, 1) + B_matrix.transpose() @ ut.reshape(-1, 1)
        
        residual = y.reshape(-1, 1) - h * f_val - rhs.flatten().reshape(-1, 1)
        
        if np.max(np.abs(residual)) < be_tol:
            break
        ...
        # J * delta_y = -residual
        delta_y = np.linalg.solve(J, residual_np)
        # np.set_printoptions(precision=12, suppress=True)
        # print("J: ",J)
        # print("residual: ",residual_np.transpose())
        # print("delta_y: ", delta_y.transpose())
        # print("y: ", y.transpose())
        y = y - delta_y
        
        iter_count += 1
    
    if iter_count >= N_ITER_MAX_NEWTON:
        print(f'Warning! Reach the maximum number of iterations ({N_ITER_MAX_NEWTON})')
...
def jfnk(f_eval, t, y0,A_matrix, B_matrix, y_approx, S, S_p, spectral_radius, n_iter_max_newton=N_ITER_MAX_NEWTON, \
    ...
    y_init, Y_init, D_init, is_converged, is_stiff, ratios \
        = jfnk_initial(f_eval, t, y0,A_matrix, B_matrix, y_approx, S, S_p, spectral_radius, be_tol=be_tol, sdc_tol=sdc_tol)
    ...
    if (not is_converged) and (is_stiff):
        y, Y, D, is_converged = jfnk_iterations(f_eval, t, y0,A_matrix, B_matrix, y_init, S, S_p, n_iter_max_newton, \
...
def jfnk_adaptive(f_eval, t_init, t_final, dt_init, p, y0, A_matrix, B_matrix, tol, n_iter_max_newton=N_ITER_MAX_NEWTON, be_tol=BE_TOL, \
    ...
    while (t0 < t_final) and (not do_stop):
        ...
        t, y, Y, D = jfnk_uniform(f_eval, t_init=t0, t_final=(t0 + h), n_steps=1, p=p, y0=v0, A_matrix=A_matrix, B_matrix=B_matrix, \
    ...
    print("run_sdc_sweep_num: ", run_sdc_sweep_num)
    print("run_jfnk_initial_num: ", run_jfnk_initial_num)
    print("run_jfnk_iteratin_num: ", run_jfnk_iteratin_num)
    if do_print:
        print_elapsed_time(start, end)
...
def jfnk_initial(f_eval, t, y0,A_matrix, B_matrix, y_approx, S, S_p, spectral_radius, be_tol=BE_TOL, sdc_tol=SDC_TOL, \
                 n_iter_max=N_ITER_MAX_SDC, do_print=False):
    global run_jfnk_initial_num
    run_jfnk_initial_num += 1
    ...
    y_sdc, Y_sdc, D_sdc, is_converged = sdc(f_eval, t, y0,A_matrix, B_matrix, y_approx, S, S_p, n_iter_max_sdc=2, be_tol=be_tol, \
    ...
    while (not is_converged) and (not is_stiff) and (i < n_iter_max):
        ...
        # print("is_stiff: ", ratio / spectral_radius)
        ...
        if is_stiff:
            ...
            y_sdc, Y_sdc, D_sdc, is_converged = sdc(f_eval, t, y0,A_matrix, B_matrix, y_sdc, S, S_p, n_iter_max_sdc=1, \
                                                    be_tol=be_tol, sdc_tol=sdc_tol, do_print=False)
...
def jfnk_iterations(f_eval, t, y0,A_matrix, B_matrix,  y_init, S, S_p, n_iter_max_newton=N_ITER_MAX_NEWTON, be_tol=BE_TOL, sdc_tol=SDC_TOL):
    global run_jfnk_iteratin_num
    run_jfnk_iteratin_num += 1
    """
    ...
    while (not is_converged) and (k < n_iter_max_newton):
        ...
        y, Y, D, is_converged = sdc(f_eval, t, y0,A_matrix, B_matrix, y, S, S_p, n_iter_max_sdc=k_sdc,\
                                    be_tol=be_tol, sdc_tol=sdc_tol)
...
def jfnk_step(f_eval, p, dt, t, y0,A_matrix, B_matrix,  n_iter_max_newton=N_ITER_MAX_NEWTON, be_tol=BE_TOL, \
              sdc_tol=SDC_TOL, do_print=False):
    """
    ...
    y_be = backward_euler(f_eval, t, y0,A_matrix, B_matrix, S_p, be_tol=be_tol)
    # np.set_printoptions(precision=12, suppress=True)
    # np.set_printoptions(precision=12, suppress=True)
    # print("y_be")
    # print(y_be)
    ...
    # JFNK solver
    y, Y, D, is_converged, is_stiff, ratios = jfnk(f_eval, t, y0,A_matrix, B_matrix, y_be, S, S_p, spectral_radius, \
...
def jfnk_uniform(f_eval, t_init, t_final, n_steps, p, y0,A_matrix, B_matrix, n_iter_max_newton=N_ITER_MAX_NEWTON, \
                 be_tol=BE_TOL, sdc_tol=SDC_TOL, do_print=False):
    ...
    y, Y, D, is_converged, is_stiff, ratios = jfnk_step(f_eval, p, dt, nodes, v0,A_matrix, B_matrix, n_iter_max_newton, \
                                                        be_tol=be_tol, sdc_tol=sdc_tol)
...
def sdc(f_eval, t, y0,A_matrix, B_matrix, y_old, S, S_p, n_iter_max_sdc=N_ITER_MAX_SDC, be_tol=BE_TOL, \
        sdc_tol=SDC_TOL, do_print=False):
    ...
    while (not is_converged) and  (k < n_iter_max_sdc):
        ...
        y_sdc, d = sdc_sweep(f_eval, t, y0,A_matrix, B_matrix, y_sdc, F, S, S_p, be_tol=be_tol)
    ...
    # print("\n")
    # y_sdc是最新迭代结果，Y是所有迭代结果，Y中不包含y_sdc
    
    return y_sdc, Y, D, is_converged

def sdc_node(f_eval, t, h, rhs, y_old,A_matrix, B_matrix, be_tol=BE_TOL):
    ...
    y_new = backward_euler_node(f_eval, t, h, rhs=rhs, y0=y_old,A_matrix=A_matrix, B_matrix=B_matrix, be_tol=be_tol)
...
def sdc_sweep(f_eval, t, y0,A_matrix, B_matrix, y_old, F, S, S_p, be_tol=BE_TOL):
    ...
    for i in range(n_nodes):
        ...
        if i == 0 and do_skip:
            ...
            y_temp, d_temp = sdc_node(f_eval, t[i], h, rhs, y_old[i],A_matrix, B_matrix, be_tol=be_tol)
