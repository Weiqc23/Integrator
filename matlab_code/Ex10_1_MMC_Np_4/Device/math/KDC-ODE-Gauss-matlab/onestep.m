...
%    line 122: lcount==4, how many nondecay case happened.
%
...
y0_block = ones(n+1,1)*y0;
delta0_block=zeros(m*(n+1),1); % the initial guess
params=[gtol,k0]; %GMRES parameters.

global nodeType use_gmres
global check_interpolate
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ynew=predictor(y0,t,m,n, kmax, etol);    
    ...
    while count1<kmax% && errest>etol %kmax: max number of iterations allowed.
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      epsilon=updated(rhs,t,dfdy,n,m);  % apply the preconditioner to the right hand side.
      % epsilon=updated_mex(rhs,t,dfdy,n,m);
      if (convergence_criteria(epsilon,ynew,etol))
          break;
      end
    ...
    if (check_interpolate)
        [t1, y, t, ynew, some_state_judge_wrong] = detectAndInterpolate_SDC(t1, y, t, ynew);
        [z, znew] = GetOutputs_SDC(t1, y, t, ynew);
        if (some_state_judge_wrong)
            [t1,y,z, t,ynew,znew]=onestep(m,t0,y0,dt,n,tc,kmax,gtol,etol,k0,A,B);
            return;
        end
    else
        [z, znew] = GetOutputs_SDC(t1, y, t, ynew);
